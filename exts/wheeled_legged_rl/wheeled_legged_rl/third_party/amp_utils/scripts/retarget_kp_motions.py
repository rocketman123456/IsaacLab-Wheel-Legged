...
from wheeled_legged_rl.third_party.amp_utils.kinematics.urdf import build_serial_chain_from_urdf
...
def build_markers(num_markers):
    ...
    for i in range(num_markers):
        ...
        virtual_shape_id = pybullet.createVisualShape(shapeType=pybullet.GEOM_SPHERE, radius=marker_radius, rgbaColor=col)
...
def get_tar_toe_pos_local(pose):
    return pose[(POS_SIZE + ROT_SIZE + JOINT_POS_SIZE) : (POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE)]


def get_linear_vel(pose):
    return pose[
        (POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE) : (POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE + LINEAR_VEL_SIZE)
    ]


def get_angular_vel(pose):
    return pose[
        (POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE + LINEAR_VEL_SIZE) : (
            POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE + LINEAR_VEL_SIZE + ANGULAR_VEL_SIZE
        )
    ]
...
def get_joint_vel(pose):
    return pose[
        (POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE + LINEAR_VEL_SIZE + ANGULAR_VEL_SIZE) : (
            POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE + LINEAR_VEL_SIZE + ANGULAR_VEL_SIZE + JOINT_VEL_SIZE
        )
    ]
...
def get_tar_toe_vel_local(pose):
    return pose[
        (POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE + LINEAR_VEL_SIZE + ANGULAR_VEL_SIZE + JOINT_VEL_SIZE) : (
            POS_SIZE + ROT_SIZE + JOINT_POS_SIZE + TAR_TOE_POS_LOCAL_SIZE + LINEAR_VEL_SIZE + ANGULAR_VEL_SIZE + JOINT_VEL_SIZE + TAR_TOE_VEL_LOCAL_SIZE
        )
    ]
    ...
    tar_toe_pos_local = np.squeeze(
        np.concatenate(
            [
                ...
                chain_foot_fr.forward_kinematics(joint_pose[3:6]).get_matrix()[:, :3, 3],
                chain_foot_rl.forward_kinematics(joint_pose[6:9]).get_matrix()[:, :3, 3],
                chain_foot_rr.forward_kinematics(joint_pose[9:12]).get_matrix()[:, :3, 3],
            ],
            axis=-1,
        )
    )
...
def retarget_motion(robot, joint_pos_data):
    ...
    for f in range(num_frames - 1):
        ...
        if f == 0:
            pose_size = curr_pose.shape[-1] + LINEAR_VEL_SIZE + ANGULAR_VEL_SIZE + JOINT_POS_SIZE + TAR_TOE_VEL_LOCAL_SIZE
...
def main(argv):
    ...
    if config.VISUALIZE_RETARGETING:
        ...
        p.connect(p.DIRECT, options='--width=1920 --height=1080 --mp4="test.mp4" --mp4fps=60')
    ...
    for mocap_motion in config.MOCAP_MOTIONS:
        ...
        set_pose(
            robot,
            np.concatenate([config.INIT_POS, config.INIT_ROT, config.DEFAULT_JOINT_POSE]),
        )
        ...
        # 读取每一行为一个数组
        joint_pos_data = load_ref_data(mocap_motion[1], mocap_motion[2], mocap_motion[3] + 1)
        ...
        for ref_toe_joint_id in REF_TOE_JOINT_IDS:
            ...
            joint_pos_data[:, ref_toe_joint_id, -1] -= np.min(joint_pos_data[:, ref_toe_joint_id, -1])
        ...
        if config.VISUALIZE_RETARGETING:
            ...
            for repeat in range(1 * num_frames):
                ...
                foot_line_unique_ids = set_foot_marker_pos(get_tar_toe_pos_local(pose), robot, foot_line_unique_ids)
                linear_vel_unique_id = set_linear_vel_pos(get_linear_vel(pose), robot, linear_vel_unique_id)
                angular_vel_unique_id = set_angular_vel_pos(get_angular_vel(pose), robot, angular_vel_unique_id)
